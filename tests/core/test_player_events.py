...
def test_a_visible_move_produces_a_chip_event() -> None:
    ...
    (event,) = engine.player_events("move", facts)
    moved = facts[-1]
...
def test_move_chip_titles_read_who_moved_and_who_or_where_received_it() -> None:
    ...
    (taken,) = engine.player_events("move", tuple(actions.move(draft, VAULT_MAP, PLAYER_ID)))
    assert taken.title == "Took the vault map"

    (given,) = engine.player_events("move", tuple(actions.move(draft, VAULT_MAP, MARA)))
    assert given.title == "Gave the vault map to Mara"

    (left,) = engine.player_events("move", tuple(actions.move(draft, LANTERN, STUDY)))
...
def test_advance_thread_produces_no_event() -> None:
    ...
    assert engine.player_events("advance_thread", facts) == ()
...
def test_completing_the_chapter_produces_a_chip_event() -> None:
    ...
    (event,) = engine.player_events("complete_chapter", facts)
...
def test_gain_improvised_item_produces_a_took_chip_event() -> None:
    ...
    (event,) = engine.player_events("gain_improvised_item", facts)
...
def test_the_reveal_tool_produces_a_discovered_chip_event() -> None:
    ...
    (event,) = engine.player_events("reveal", tuple(actions.reveal(draft, VAULT_MAP)))
...
def test_the_remaining_core_chips_carry_id_free_titles() -> None:
    ...
    (unlocked,) = engine.player_events("unlock_exit", tuple(actions.unlock_exit(draft, VAULT)))
    assert unlocked.title == "The sealed vault unlocked"

    (joined,) = engine.player_events("join_party", tuple(actions.join_party(draft, TOMAS)))
    assert joined.title == "Brother Tomas joins your party"

    (left,) = engine.player_events("leave_party", tuple(actions.leave_party(draft, TOMAS)))
    ...
    (added,) = engine.player_events("add_trait", added_facts)
    assert added.title == "Brother Tomas gained Wary"

    removed_facts = tuple(actions.remove_trait(draft, TOMAS, "wary"))
    (removed,) = engine.player_events("remove_trait", removed_facts)
...
def test_a_tool_call_on_an_unrevealed_entity_produces_nothing() -> None:
    ...
    assert all(fact.narrator is None for fact in facts)
    assert engine.player_events("add_trait", facts) == ()

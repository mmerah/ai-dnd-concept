...
def test_top_level_consequences_all_apply_in_order(state: GameState) -> None:
    ...
    assert isinstance(moved, Moved) and (moved.subject_id, moved.location_id) == ("player", "vault")
...
def test_take_a_present_item_reveals_it_and_moves_it_to_the_player(state: GameState) -> None:
    ...
    assert (took.item_id, took.item_name, took.to_kind) == ("vault_map", "the vault map", "player")
...
def test_give_hands_a_held_item_to_a_present_npc(state: GameState) -> None:
    give = GiveItem(item_id=EntityId("lantern"), npc_id=EntityId("mara"))
    (given,) = resolve([give], state, PASS)
    assert isinstance(given, ItemMoved)
    assert (given.item_id, given.to_id, given.to_kind) == ("lantern", "mara", "npc")


def test_give_to_an_absent_npc_fails(state: GameState) -> None:
        ...
        world=updated(
            ...
            entities={
                **state.world.entities,
                EntityId("mara"): updated(
                    state.world.entities[EntityId("mara")], location_id=EntityId("vault")
                ),
            },
        ),
    )
    with pytest.raises(ValueError, match="not at the player's location"):
        resolve([GiveItem(item_id=EntityId("lantern"), npc_id=EntityId("mara"))], away, PASS)
...
def test_move_the_player_to_a_hidden_location_reveals_it(state: GameState) -> None:
    ...
    assert isinstance(moved, Moved) and (moved.subject_id, moved.location_name) == (
        "player",
        "the vault",
    )
...
def test_move_a_known_npc_away_does_not_reveal(state: GameState) -> None:
    move = Move(location_id=EntityId("vault"), subject_id=EntityId("mara"))
    (moved,) = resolve([move], state, PASS)
    assert isinstance(moved, Moved) and (moved.subject_id, moved.location_id) == ("mara", "vault")


def test_move_a_hidden_npc_into_the_room_reveals_it(state: GameState) -> None:
    """Elena is hidden and at the player's location; moving her here reveals her arrival."""
    arrive = Move(location_id=EntityId("study"), subject_id=EntityId("elena"))
    events = resolve([arrive], state, PASS)
    assert [e.type for e in events] == ["entity_discovered", "moved"]


def test_moving_an_npc_the_player_cannot_witness_fails(state: GameState) -> None:
    ...
    move = Move(location_id=EntityId("study"), subject_id=EntityId("mara"))
...
def test_a_bind_made_inside_a_check_branch_does_not_leak_to_a_later_sibling(
    ...
    mechanics: list[Consequence] = [
        RollCheck(ability="wisdom", dc=12, on_success=[RollDice(dice="1d6", bind="dmg")]),

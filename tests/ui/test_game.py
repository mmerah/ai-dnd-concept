...
from aidm.ui.game import _can_type  # pyright: ignore[reportPrivateUsage]
...
def _pick(*, allows_text: bool) -> PendingDecision:
    return PendingDecision(
        kind="pick",
        prompt="Which door?",
        options=(PendingOption(id="left", label="Left"),),
        allows_text=allows_text,
    )


def test_the_composer_opens_only_between_turns_on_a_game_still_going() -> None:
    assert _can_type(_view(), None)
    assert not _can_type(_view(), "master")
    assert not _can_type(_view(prompt=_pick(allows_text=False)), None)
    assert _can_type(_view(prompt=_pick(allows_text=True)), None)
    assert not _can_type(_view(over="Wren is dead"), None)

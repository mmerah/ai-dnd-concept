...
def test_a_level_preview_lists_every_level_two_gain_before_advancing() -> None:
    preview = progression.preview(player_of(new_game()), RULES)
...
def test_a_constitution_increase_adds_hp_for_every_existing_level() -> None:
    ...
    third = player_of(levelled(new_game(), 3))
    plan = progression.plan(third, decisions, RULES)
    assert plan.benefits.retroactive_hp_gain == 3

    con_facts = progression.advance(player_of(levelled(new_game(), 3)), decisions, RULES, Random(1))
    strength_facts = progression.advance(
        player_of(levelled(new_game(), 3)),
        {"ability-scores-4": ("strength", "strength")},
        RULES,
        Random(1),
    )
...
def test_a_level_choice_does_not_offer_a_feature_already_held() -> None:
    player = player_of(levelled(new_game(), 9))

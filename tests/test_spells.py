...
import pytest
from support import new_game, ruleset
from test_progression import levelled, started

from aidm.content.records.base import ContentRef
from aidm.domain.models.base import EntityId
from aidm.domain.models.consequences import Cast
from aidm.domain.models.entities import ActorEntity
from aidm.domain.models.events import DcRolled, DiceRolled, Event
from aidm.domain.models.progression import Progression
from aidm.domain.models.state import GameState
from aidm.domain.reducer import apply, render
from aidm.engine import bestiary, spells
...
def guarded(klass: str, level: int = 1) -> GameState:
    ...
    state = levelled(started(klass, new_game("whispering_vault")), level)
...
def test_a_save_that_halves_on_success_halves_rather_than_skips() -> None:
    ...
    # The Narrator's only view of it names no hit points, absolute or maximum.
    assert str(GARGOYLE_HP) not in render(saved) and "a gargoyle is hurt" in render(saved)

...
def test_attempt_bands_disaster_setback_success() -> None:
    ...
    facts = _rolled(draft, what="Slip past", skill="Stealth", seed=2)
    assert facts[1].trace.endswith("→ disaster")

    draft = small_world().draft()
    facts = _rolled(draft, what="Slip past", skill="Stealth", seed=1)
    assert facts[1].trace.endswith("→ setback")

    draft = small_world().draft()
    facts = _rolled(draft, what="Slip past", skill="Stealth")
    assert facts[1].trace.endswith("→ success")


def test_attempt_unskilled_rolls_the_plain_d6(draft: TwentyfourxxGame) -> None:
    facts = _rolled(draft, what="Guess")
...
def test_attempt_pack_label_not_on_sheet_rolls_d6(draft: TwentyfourxxGame) -> None:
    facts = _rolled(draft, what="Scale the wall", skill="Climbing")
    assert facts[1].dice[0].faces == (6,)
...
def test_attempt_unknown_skill_refused_with_both_lists(draft: TwentyfourxxGame) -> None:
    with pytest.raises(Refusal) as raised:
        _ = _rolled(draft, what="Try", skill="Nonexistent")
...
def test_attempt_helped_by_rolls_two_dice_and_keeps_highest(draft: TwentyfourxxGame) -> None:
    ...
    facts = _rolled(draft, what="Slip past", skill="Stealth", helped_by=Helper(actor_id=KESTREL))
...
def test_attempt_actor_id_acts_on_the_member_and_risk_kills_them(draft: TwentyfourxxGame) -> None:
    ...
    facts = _rolled(
        draft, what="Slip past", actor_id=KESTREL, skill="Stealth", risk="a long fall", seed=2
    )
...
def test_risk_kills_on_disaster_and_maims_on_setback_not_doubled() -> None:
    ...
    facts = _rolled(draft, what="Sneak past", skill="Stealth", risk="a guard's knife", seed=1)
    assert player.alive
    assert player.require_sheet().hindrances == ["Maimed"]
    assert any(fact.card == "Maimed" for fact in facts)

    _ = _rolled(draft, what="Sneak past", skill="Stealth", risk="a guard's knife", seed=1)
    assert player.require_sheet().hindrances == ["Maimed"]

    draft = small_world().draft()
    player = draft.payload.player
    facts = _rolled(draft, what="Sneak past", skill="Stealth", risk="a guard's knife", seed=2)
    assert not player.alive
    assert any(fact.card == "You are dead" for fact in facts)


def test_defend_with_intact_item_spares_a_disaster_breaks_the_item_once(
...
def test_defend_with_harmless_gear_spares_a_disaster_and_adds_no_hindrance(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
    facts = _rolled(
...
def test_defend_with_already_broken_gear_is_refused_before_any_dice_roll(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_defend_with_multi_use_armor_defends_three_times_then_refuses(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
    ...
    for _ in range(3):
        _ = _rolled(
            draft, what="Take fire", skill="Stealth", risk="a bullet", defend_with="armor", seed=2
        )
    assert player.alive
...
def test_defend_with_non_harmless_multi_use_armor_takes_a_different_hindrance_each_time(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_defend_with_non_harmless_gear_and_no_hindrance_is_refused_before_any_dice_roll(
    draft: TwentyfourxxGame,
) -> None:
    before = draft.model_copy(deep=True)
...
def test_helper_defends_with_their_own_gear_while_actor_takes_their_own_consequence(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Stealth": 10}).draft()
...
def test_both_participants_naming_the_same_ship_function_are_refused_before_any_dice_roll(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Stealth": 10}).draft()
    before = draft.model_copy(deep=True)
...
def test_helper_with_risk_takes_their_own_consequence_on_a_bad_roll(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Stealth": 10}).draft()
    facts = _rolled(
...
def test_helper_with_no_risk_is_untouched_on_a_bad_roll(draft: TwentyfourxxGame) -> None:
    draft = hired(draft, KESTREL, skills={"Stealth": 10}).draft()
    _ = _rolled(
        draft, what="Slip past", skill="Stealth", helped_by=Helper(actor_id=KESTREL), seed=2
    )
    member = draft.payload.cast[KESTREL]
    assert draft.payload.player.alive
    assert member.alive
...
def test_succession_runs_only_after_both_the_actor_and_helper_consequences_land(
    draft: TwentyfourxxGame,
) -> None:
    state = hired(draft, KESTREL, skills={"Stealth": 10})
...
def test_defend_breaks_the_item_and_adds_the_hindrance_refused_when_broken(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_repair_item_zeroes_broken_times_and_refuses_an_unbroken_item(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_change_hindrances_gains_and_loses_refuses_duplicate_and_absent(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_finish_job_raises_a_skill_enters_a_new_one_refuses_at_d12_adds_credits(
    draft: TwentyfourxxGame,
) -> None:
    player = draft.payload.player
...
def test_finish_job_names_lowercase_stealth_and_raises_the_existing_key(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Stealth": 8}).draft()
    member = draft.payload.cast[KESTREL]
...
def test_finish_job_names_a_hired_members_skill_when_already_at_d12(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Shooting": 12}).draft()
    draft.payload.job = "Escort the crate"
...
def test_android_case_is_an_item_on_creation_and_defend_breaks_it_harmlessly(
    draft: TwentyfourxxGame,
) -> None:
    picks = {
        "specialty": "tech",
...
def test_risk_disaster_with_hired_member_sets_succession_and_over_stays_none(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Shooting": 8}).draft()
    facts = _rolled(draft, what="Slip past", skill="Stealth", risk="a long fall", seed=2)
    assert not draft.payload.player.alive
    assert draft.pending is not None
    assert draft.pending.kind == "succession"
...
def test_kill_on_the_lead_with_a_hired_member_opens_the_succession(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Shooting": 8}).draft()
...
def test_risk_disaster_with_none_hired_ends_the_game(draft: TwentyfourxxGame) -> None:
    _ = _rolled(draft, what="Slip past", skill="Stealth", risk="a long fall", seed=2)
    assert not draft.payload.player.alive
    assert draft.pending is None
    assert ENGINE.over(draft) == "You died."


def test_answering_the_succession_decision_makes_the_member_the_player(
    draft: TwentyfourxxGame,
) -> None:
    draft = hired(draft, KESTREL, skills={"Shooting": 8}).draft()
    _ = _rolled(draft, what="Slip past", skill="Stealth", risk="a long fall", seed=2)

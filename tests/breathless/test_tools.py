...
def test_stunt_refused_twice() -> None:
    ...
    with pytest.raises(ValueError, match="catches their breath"):
        _ = check(draft, Check(what="Leap again", stunt=True), Random(0))
...
def test_use_med_kit_refused_without_a_kit() -> None:
    ...
    with pytest.raises(ValueError, match="holds no med kit"):
        _ = use_med_kit(draft, NoArgs(), Random(0))
...
def test_change_stress_refuses_a_zero_amount() -> None:
    ...
    with pytest.raises(ValueError, match="non-zero"):
        _ = change_stress(draft, ChangeStress(amount=0, why="nothing"), Random(0))


def test_loot_1_or_2_leaves_a_note_and_no_pending() -> None:
    draft = small_world().draft()
    facts = loot_check(draft, LootCheck(item="Rope"), Random(2))
...
def test_loot_on_an_item_with_room_offers_take() -> None:
    draft = small_world().draft()
    _ = loot_check(draft, LootCheck(item="Crowbar"), Random(0))
...
def test_loot_on_an_item_with_a_full_backpack_offers_swaps() -> None:
    ...
    _ = loot_check(draft, LootCheck(item="Crowbar", granted=8, choice="swap-rope"), Random(0))
...
def test_loot_at_d10_or_better_also_offers_a_med_kit() -> None:
    draft = small_world().draft()
    _ = loot_check(draft, LootCheck(item="Shotgun"), Random(17))
    assert draft.pending is not None
...
def test_luck_facts_are_untold() -> None:
    draft = small_world().draft()
    dice_fact, luck_fact = roll_luck(draft, LuckTest(question="Is anyone home?", die=6), Random(0))
...
def test_leave_and_enter_on_the_player_are_refused() -> None:
    draft = small_world().draft()
    assert "in every scene" in refused(draft, "leave", entity_id=PLAYER_ID)
    assert "in every scene" in refused(draft, "enter", entity_id=PLAYER_ID)

...
- An engine is self-contained under `engines/<id>/`, under 2,000 lines, with at most fifteen game-master tools, world verbs included, one per SRD procedure. The scene engines share the scene lifecycle in `engines/scenes.py`.
- Only the narrator writes player-facing text. Its input holds revealed facts only. Hidden facts have no path into it.

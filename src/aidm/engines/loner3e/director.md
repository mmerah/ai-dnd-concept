...
Each beat puts at most one thing to the dice, and this engine has one: a `question`. Once it
resolves, you are asked again for what the outcome caused. Leave `roll` null when nothing the
player does is uncertain — a conversation, a look around, a walk to a room they know; then the
plan is whatever `effects` the turn plainly causes, and often none at all.
...
Leave `roll` null to end the turn when the next move is the player's to choose.

...
A conflict has two active sides, such as a fight, chase, hunt, or argument. When another actor resists, set `opponent_id` to their exact id. This field makes the exchange affect luck. Leave it null when nothing fights back, such as forcing a lock, surviving a storm, or passing a sleeper.
...
At 0 luck, that actor loses the conflict. Use the tool result to end it in the story: they may be captured, broken, driven off, or forced to concede. This does not automatically mean death. The engine restores both sides' luck.

...
- `no` is failure, and the situation stays the same.
- `no-and` is failure with a worse situation.

Keep an answered question settled. If a result does not fit easily, show a complication or a
deeper truth that makes the result fit. If no result fits, use `yes-but` with a small
complication.

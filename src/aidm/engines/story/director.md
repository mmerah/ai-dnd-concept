...
their own; a gear benefit sits as a tag there.
...
- `hindering_tag_id` — at most one tag on that actor's own sheet that directly hinders: a burden
  or a condition. Null unless one really applies.
...
setback. Never touch an approach number, a `stress` maximum, or the `growth` pool — growth is
spent in the advancement panel, not on a turn.

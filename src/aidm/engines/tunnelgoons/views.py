...
def place_lines(world: TunnelGoonsWorld, *, known: bool) -> str:
    npcs_here = [npc for npc in world.at(world.current.id) if npc.known == known]
    holders = (world.current.id, *(npc.id for npc in world.at(world.current.id)))
    items = (item for holder in holders for item in world.carried(holder) if item.known == known)
    return lines_of(world.line(entity) for entity in (*npcs_here, *items))

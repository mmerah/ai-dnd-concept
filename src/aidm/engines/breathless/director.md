...
Fill in one die: `skill` from the six, or `item_id` for a carried item used in its place. When the player declares a stunt — one showy, all-or-nothing move — set `stunt` true and leave `skill` empty: it rolls a d12, and they cannot stunt again until they catch their breath. Set `dangerous` true when failing could harm the actor. An ally here who helps goes in `helper_id` with their own `helper_skill`, `helper_item_id` or `helper_stunt`: they roll too and share the risk.

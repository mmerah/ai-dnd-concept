...
def offered(engine: Engine, state: GameState) -> AdvancementOffer | None:
    ...
    if player.trait(ADVANCEMENT_READY) is None and not _milestone_reached(state, sheet):
        return None
    record = engine.record(level_ref(sheet, sheet.numbers[LEVEL] + 1))
    if record is None:
...
def advance(engine: Engine, draft: GameState, proposal: ProposalBase) -> tuple[Fact, ...]:
    del engine
...
def check_proposal(
    engine: Engine, state: GameState, offer: AdvancementOffer, proposal: ProposalBase
) -> str | None:
    assert isinstance(proposal, LevelUp)

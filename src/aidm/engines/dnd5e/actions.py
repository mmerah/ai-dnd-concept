...
class Attack(Frozen):
    ...
    weapon_item_id: EntityId | None = Field(
        default=None,
        description=(
            "Exact id of a weapon item the attacker carries. Null when the attack comes from "
            "a stat block instead."
        ),
    )
    attack_bonus: Annotated[int, Field(ge=-10, le=30)] | None = Field(
        default=None,
        description=(
            "To-hit bonus copied off a stat block's attack line, such as 4 for `Bite +4 to "
    ...
    damage: DiceExpr | None = Field(
        default=None,
        description=(
            "Damage dice copied off that same line, such as `1d4+2`. Null whenever a weapon "
            "item is named."
        ),
    )
...
class CastSpell(Frozen):
    ...
    slot_level: Annotated[int, Field(ge=1, le=9)] | None = Field(
        default=None,
        description=(
            "The spell-slot level it is cast from; a higher one upcasts it. Null for a "
    ...
    target_id: EntityId | None = Field(
        default=None,
        description=(
            "Exact id of the one creature it is aimed at, here with the player. Null only "
...
class Check(Frozen):
    ...
    bonus: int = Field(
        ...
        le=30,
        description=(
            "The actor's own bonus: their ability modifier, plus `proficiency-bonus` only "
...
class UseFeature(Frozen):
    ...
    heal: DiceExpr | None = Field(
        default=None,
        description=(
            "The healing dice the feature's own text gives, bonuses included, such as "
...
class Improvise(Frozen):
    ...
    vs: Annotated[int, Field(ge=1, le=40)] | None = Field(
        default=None,
        description="The number the total must reach; null when nothing is contested.",
    )

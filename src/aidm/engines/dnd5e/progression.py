...
def _option_available(
    ...
    held_proficiencies: set[ContentSlug],
) -> bool:
...
def _proficiencies(held: Sequence[ContentSlug], picks: Sequence[Pick]) -> tuple[ContentSlug, ...]:
...
def _proficiency_picks(picks: Sequence[Pick], effect: ChoiceEffect) -> list[ContentSlug]:
    return [
        p.option.ref.index
        for p in picks
...
def _hp_gain(rolled: int, attributes: Attributes) -> int:
    return max(1, rolled + rolls.modifier(attributes, "constitution"))


def _retroactive_hp_gain(levels: int, before: Attributes, after: Attributes) -> int:
    gained_modifier = rolls.modifier(after, "constitution") - rolls.modifier(before, "constitution")

...
def _walk(
    ...
    match consequence:
        case RollCheck(ability=ability, dc=dc, on_success=on_success, on_failure=on_failure):
            rolled = rules.roll_check(draft.character, ability, dc, rng)
        ...
        case Damage(amount=amount):
            return [HpChanged(delta=-_value(amount, bindings))]
        ...
        case Move(location_id=location_id, subject_id=subject_id):
            return _move(draft, location_id, subject_id)
        case TakeItem(item_id=item_id):
            item = _item(draft, item_id)
            if item.location_id != draft.character.location_id:
                raise ValueError(f"cannot take {item_id!r}: it is not at the player's location")
            took = _item_moved(item, PLAYER_ID, draft.character.name, "player")
            return [*_reveal(item), took]
        case DropItem(item_id=item_id):
            item = _held(draft, item_id, "drop")
            here = _location(draft, draft.character.location_id)
            return [_item_moved(item, here.id, here.name, "location")]
        case GiveItem(item_id=item_id, npc_id=npc_id):
            item = _held(draft, item_id, "give")
            npc = _npc(draft, npc_id)
            if npc.location_id != draft.character.location_id:
                raise ValueError(f"cannot give to {npc_id!r}: not at the player's location")
            return [_item_moved(item, npc.id, npc.name, "npc")]
        case GainImprovisedItem(item_name=item_name):
            item = make_entity(
                ...
                location_id=draft.character.location_id,  # created lying here, then picked up
                known=True,
                authored=False,
            )
            took = _item_moved(item, PLAYER_ID, draft.character.name, "player")
...
def _move(draft: GameState, location_id: EntityId, subject_id: EntityId | None) -> list[Event]:
    """The player, or a named NPC, walks to a location. An NPC move must touch the player's location
    — they watch it leave or arrive — so the summary never narrates movement the player could not
    witness; arriving where the player is also reveals the NPC."""
    dest = _location(draft, location_id)
    here = draft.character.location_id
    if subject_id is None:
        moved = Moved(
            subject_id=PLAYER_ID,
            subject_name=draft.character.name,
            location_id=dest.id,
            location_name=dest.name,
        )
        return [*_reveal(dest), moved]
    npc = _npc(draft, subject_id)
    if npc.location_id != here and dest.id != here:
        raise ValueError(f"cannot move {subject_id!r}: the player would not witness it")
    reveal = _reveal(npc) if dest.id == here else []
    moved = Moved(
        subject_id=npc.id, subject_name=npc.name, location_id=dest.id, location_name=dest.name
    )
    return [*reveal, moved]


def _item_moved(
    item: Entity, to_id: EntityId, to_name: str, to_kind: Literal["player", "npc", "location"]
) -> ItemMoved:
    return ItemMoved(
...
def _npc(state: GameState, entity_id: EntityId) -> NpcEntity:
    entity = _entity(state, entity_id)
    if not isinstance(entity, NpcEntity):
        raise ValueError(f"mechanics used {entity_id!r} as an npc, but it is a {entity.kind}")
    return entity
...
def _held(state: GameState, entity_id: EntityId, verb: str) -> ItemEntity:
    ...
    if entity_id not in state.character.inventory:

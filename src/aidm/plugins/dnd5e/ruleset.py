...
class SpellProfile(Value):
    ref: ContentRef
    name: str
    level: SpellLevel


class FeatureProfile(Value):
    ref: ContentRef
    name: str
    desc: str
...
class AttackProfile(Value):
    name: str
    to_hit: int
    damage: dice.SelfContainedDice | None = None


class WeaponProfile(Value):
    index: ContentSlug
...
class Ruleset(ProgressionRules, Protocol):

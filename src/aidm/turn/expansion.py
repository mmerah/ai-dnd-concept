...
def written(patch: ExpansionPatch) -> str:
    ...
    lines = [_written_entity(entity) for entity in patch.entities]
...
def _written_entity(entity: Entity) -> str:
    line = f"- {entity.id} ({entity.kind}) — {entity.name}"
    if entity.detail is not None and entity.detail.when_reached:
        return f"{line}\n  when reached: {entity.detail.when_reached}"
    return line

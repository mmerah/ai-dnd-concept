...
- `hooks`: only when the new canon needs a consequence nothing else would produce. `match` waits for a fact — `{"kind": "entity_discovered", "entity_id": "..."}` is the workhorse — and `effects` fire when it commits. Most expansions need none.

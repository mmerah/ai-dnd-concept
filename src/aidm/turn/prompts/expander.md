...
- `relations`: `kind: "connected"` joins two locations the player can walk between. A location you add is unreachable until a `connected` relation joins it to one that already exists — write that relation in the same patch, normally to the anchor.

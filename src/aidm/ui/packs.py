...
class PackEditor:
    ...
    def __init__(self, runtime: Runtime, engine_id: EngineId, pack_id: Slug) -> None:
        ...
        self.boxes: dict[Slug, ui.textarea] = {}
    ...
    def build(self) -> None:
        ...
        with page_body():
            ...
            with ui.card().classes("w-full"):
                for field in engine.edit_fields(pack):
                    box = (
                        ui.textarea(label=field.label, value=field.text)
                        .props(f"rows={field.rows} outlined")
                        .classes("w-full")
                        .style(MONOSPACE)
                    )
                    if not written:
                        box.props("readonly")
                    self.boxes[field.id] = box
                if written:

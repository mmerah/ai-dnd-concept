...
This file reproduces the complete official Creative Commons SRD in Markdown for engine reference.
The source is Evil Hat's `Fate-Condensed-SRD-CC-BY.html` from the
[official CC-BY SRD archive](https://fate-srd.com/downloads/CC-BY-SRDs.zip). Evil Hat's
[licensing page](https://fate-srd.com/official-licensing-fate) specifically identifies that archive
as the source to work from and licenses Fate Condensed under CC BY 3.0.

The HTML's wording is unchanged. Markup was converted to Markdown, empty anchors were removed,
and heading levels were nested under this document title. The source archive itself contains
unresolved `page XX` cross-references and a few malformed closing tags; the cross-references are
preserved because they are part of the official SRD source.
...
1. Roll four dice.
1. Add up the symbols on the dice: a [+] is +1, [-] is -1, and [0] is 0. This will give you a dice result of -4 to 4.
1. Add the dice result to your skill rating.
1. Modify the dice by invoking aspects (**page XX**) and using stunts (**page XX**).
1. Declare your total result, called your **effort**.
...
- **Script Change RPG Toolbox:** For something with a bit more nuance and granularity, look to Script Change by Beau Jager Sheldon, which provides options to pause, rewind, skip ahead, and more using an accessibly familiar media-player metaphor. Learn more about Script Change at [http://tinyurl.com/nphed7m](http://tinyurl.com/nphed7m)
...
existing Loner 3e and 24XX packages: typed proposals, resolver-owned rolls and state changes,
strict save validation, in-app character creation, advancement, player decisions, prompt guidance,
offline tests, and no rule parsing at run time.

### Engine shape

- **Sheet:** five named aspects; the 19-skill ladder; stunts; refresh and current fate points;
  separate physical and mental stress; mild, moderate, and severe consequences; milestone and
  breakthrough ledgers.
- **Action proposal:** one of overcome, create an advantage, attack, or defend; skill, target,
  opposition, teamwork, applicable stunt, and invoked aspect IDs. The resolver rolls four Fate
  dice and derives fail, tie, success, or success with style.
- **Aspect economy:** character, situation, boost, and consequence aspects need stable IDs,
  ownership, free-invoke counts, hostile-invoke payout, removal, and strict checks that each paid
  invoke spends exactly one fate point.
- **Player decisions:** the roll must pause after dice are visible so the player can invoke or
  accept; compels pause for accept/refuse; a hit pauses for stress, consequence, concession, or
  being taken out. These decision paths are the largest part of the implementation.
- **Extended play:** conflicts, challenges, contests, concession, recovery, session milestones,
  and arc breakthroughs belong in the core engine. Optional dials such as scale, conditions,
  weapon/armor ratings, extreme consequences, countdowns, and specialized big-bad rules stay out
  until a content pack selects them.

### Size estimate

| Deliverable | Estimated size | Why |
| --- | ---: | --- |
| Engine and rules Python | 1,400–1,800 lines | More state and decision branches than 24XX; aspects cross actions, entities, and scenes. |
| Director/advancement prompts | 100–160 lines | Fiction-first action choice, aspect permissions, compel guidance, and conflict procedure. |
| Shipped content pack | 180–260 JSON lines | The default skill list, ladder, creation defaults, stress rules, and license metadata. |
| **Expected initial diff** | **1,700–2,200 lines** | Includes package registration and golden fixtures, but not this SRD document. |

For comparison, the current engine/rules Python is 684 lines for Loner 3e and 931 lines for 24XX.
Fate is larger because the player can make a rules-significant choice after a roll, consequences
are both harm and invokable aspects, and the fate-point economy has transfers as well as spends.

### Content pack

The first and only required shipped pack should be `packs/srd.json`, named **Fate Condensed Core**.
It can be reproduced under CC BY 3.0 with the attribution at the top of this document and should
contain:

- the 19 default skills and which of the four actions each normally supports;
- the Great-to-Average starting pyramid and Mediocre default;
- refresh 3, three free stunt slots, five aspect prompts, and the default stress/consequence setup;
- the two SRD stunt templates as creation guidance, while actual aspects and stunts remain free
  text validated into typed records;
- explicit dial fields for later packs to replace the skill list, pyramid, stress, consequence,
  and stunt settings without changing resolver code.

No setting pack is required to make Fate playable: Fate Condensed is intentionally
setting-agnostic. A later original setting pack would primarily supply a different skill list,
curated stunt choices, aspect prompts, extras, and selected optional dials; scenario canon remains
in the existing scenario format rather than in the engine pack.

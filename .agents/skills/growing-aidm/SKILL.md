...
1. `begin_growth()` — returns the briefing: the bar this pass must meet, the premise or the
   source document of the scenario, and a worked example.
2. `write(patch)` and `connect(from_id, to_id)` — add locations, actors, items and threads.
...
3. `scenario_so_far()` — the whole draft as JSON. Call it whenever you lose track of the draft.
   The server holds the draft, so a compaction costs you nothing.
4. `finish_growth(summary)` — checks the draft, then makes it canon the player can discover. A

...
- Pydantic AI roles return validated structured output. Tools and output validators request retries with `ModelRetry`; turn-loop tools mutate only the turn's draft, through resolver code, and are refused against a throwaway copy first. No tool ever writes committed state, a save, or a file; an authoring tool may apply a typed patch to its in-memory draft.

...
choice if both ways exist before the player walks them. Maze Rats is a game of dungeon exploration
whose whole generative apparatus — its dungeon, wilderness and city tables — describes places and
what is in them, and an authored map is the only structure that can hold them.

**Correction, made during the phase.** An earlier draft of this paragraph claimed Maze Rats prints
"a 3-in-6 wandering check every ten dungeon minutes, travel rates by terrain, districts at 1-in-6".
**It prints none of those.** Maze Rats has three rules pages — Core Rules, Character Creation,
Character Sheet — and contains no time-keeping and no random-encounter procedure. That claim came
from `docs/NEXT-ENGINE-RESEARCH.md`, which this plan already flagged as stale, and it was built
before it was checked. The kit choice still stands on the reason above. Verify a rules claim against
the rulebook before you build on it.
...
1. `rules.py` — the procedure tools. **Six**, one per section of the SRD's own core rules:
   `danger_roll`, `reaction`, `attack`, `cast_spell`, `rest`, `level_up`. `begin_combat` folds into
   `attack`, which opens `CombatState` on the first swing. `change_world`, `move` and `unlock_way`
   are the kit's and do not count.

   **Corrected during the phase.** The first draft listed seven and named `pass_time` and `morale`.
   Both were wrong. `pass_time` implemented a dungeon clock and a wandering-monster check that Maze
   Rats does not have. `morale` is not a separate procedure: the SRD says the GM "may call for a WIL
   danger roll to see if they rout or beg for mercy", so it is `danger_roll`. `reaction` was in the
   rules all along and had been left as dead code.

...
half of P6 and P7) and the tests (P8). Every letter of PROPOSALS.md §4 is applied below; nothing is
left to decide. Line numbers are as of `1732e73` (`src` identical to `fc4d354`); find a site by the
...
|---|---|---|---|
| start | | 9,330 | |
| 1 — the seam and the two families | `*Proposal`, `check_scenario` gone, `CommissionArgs` base + concrete `commission_tool`, one `worldsmith_prompt` in `hub.py`, `on_order_lines`, hub residents moved, `apply_next/apply_job/apply_return`, `install_next/job/return`, the hub forwarders and `Engine.answer` inlined, `Subject.row`, rooms on scenes' lifecycle names | 9,235–9,270 | 9 h |
| 2 — the scaffold, the platform, the tests | concrete `create_character`/`creation_steps`, `items_shown`, Loner/24XX folds and methods, `pack.source` shown; `play`/`move_on`, `Presentation`, `save`, prompt constants, `narration_refusal`, `parse` at two boundaries, `history/scene/ready/newest/tools`, `Installed` + `FileStore.restore`, `way_on_panel` gone; nineteen tests folded, PROPOSALS.md deleted | 9,160–9,210 | 9 h |
...
1. **P6, scenes (D3 A).** `git mv src/aidm/engines/scenes/drafts.py src/aidm/engines/scenes/proposals.py`;
   ...
   to `proposal`; `draft` stays for the transactional copy (`draft: G`). Test helpers `_draft`,
   `_return_draft`, `_job_draft`, `_next_draft` in the three scene `test_worldsmith.py` and
   `tests/loner3e/test_world.py` become `_proposal`, `_return_proposal`, `_job_proposal`,
   `_next_proposal`. Grep: `Draft` under `src/aidm/engines/scenes`, `tests/core`, `tests/loner3e`,
   `tests/breathless`, `tests/twentyfourxx` finds nothing; `draft` in `scenes/worldsmith.py` and
   `scenes/proposals.py` finds nothing.
2. **P1 tail.** Delete `Engine.check_scenario` (`seam.py:186–188`) and its calls (`scenes/engine.py:115`,
   ...
   a payload with no `id` is now a bug, not a refusal). Grep: `check_scenario` finds nothing;
   `incompatible` finds nothing.
3. **P2, the commission tool.** In `seam.py` replace the `CommissionArgs` Protocol by
   ...
   description="True to have it written into the next scene or region instead of now.")`. Checked
   against the tree: `kind` is the first field in both families today, so a base keeps the property
   order `kind, brief, later`; the schemas differ only in `later`'s description, which unifies.
   `SceneCommission(CommissionArgs)` and `RoomCommission(CommissionArgs)` keep only their `kind:
   Literal[...] = Field(description=…)` line. `Engine` gains class attributes `commission_args:
...
4. **P2, one worldsmith frame (D1 A).** In `hub.py` add `SURPRISE` (moved from
   ...
   names it). `hub_sections` is not hoisted: the guard, the title and the brief differ and a hook for
   each nets no lines. Test, `tests/core/test_hub.py`: `worldsmith_prompt` renders `hub` before `world`
...
6. **P4, the world.** In `scenes/world.py` replace `apply_scene` (`:305–335`) by
   ...
   *hidden], job)`). `apply_next` lands under the open job's title or `""`; `apply_job` refuses
   ...
   or an open job, closes it, swaps the board, lands under `""`, returns the job. In
   ...
   `_next_proposal(...)`. Grep: `apply_scene` finds nothing; `isinstance` in `scenes/world.py` and
   `scenes/engine.py` finds nothing.
7. **P4, the engine.** `render_next(self, draft: G, intent: str, answer: type[BaseModel], *,
   ...
   intent, answer)` there becomes `ENGINE.advance(game, intent, answer)`; every `ENGINE.install(…)`
   in the three scene `test_worldsmith.py` becomes `install_next`/`install_job(…, reopening=None)`/
   `install_return` with the matching proposal; `render_next` calls there add `returning=False,
   follows_arc=False`. Grep: `def install(` under `engines/scenes` finds nothing; `issubclass` under
   `engines/scenes` finds nothing.
8. **P7, the engine folds.** (a) `question_heading` (`hub.py:372`) inlined into `master_sections`
   ...
   `job_row` (`:280`) into `tail`. `tests/core/test_hub.py:121–162,209–226` call `board_panel(at_hub=
   True)[0].rows`, `hub_block(title, brief, (), returning=…, reopening=None)`, `dict(tail(at_hub=
   False))["THE JOB"]`; `tests/tunnelgoons/test_views.py:68` reads `board_panel(at_hub=True)[0].rows`.
   ...
   to `name`, `detail=brief`, `icon_id=id`); `here_panel` (`base.py:143–144`) and `party_panel`
   ...
   {option.name!r} to play option {chosen!r}") from missing`, then `self._apply(lambda copy, dice:
   found.call(copy, option.args, dice))`; `tests/core/test_decisions.py:204–206` drive the same two
   refusals through `Turn.begin(engine, _pending(state, decision-with-that-option), Answer(option_id=
   "lantern"), Random(0))`, same `match`. (f) D5 A: drop the `Free:` docstring lines at `base.py:141`,
   `scenes/world.py:400`, `scenes/worldsmith.py:38,44` (each is the whole docstring). Grep: each name
   in (a)–(e) finds nothing; `Free:` under `engines/scenes` and `base.py` finds nothing.
9. **P6, rooms (D3 A).** `git mv src/aidm/engines/rooms/drafts.py src/aidm/engines/rooms/proposals.py`;
   ...
   `render_extension→render_next`, `write_extension→write_next`, `install_extension→install`
   (`install_commission` already matches); parameters `draft`/`extension` holding a proposal →
   `proposal` (`rooms/worldsmith.py` bars, `build_scenario`, `opening_canon`, `install`). Tests:
...
11. **P2, rooms adopt the frame (D1 A).** Delete `rooms/worldsmith.py:36–62`; `rooms/engine.py` imports
    ...
    `render_opening`. Tunnel Goons' prompt gains `STANDING INSTRUCTION` and its `ENGINE GUIDANCE`
    moves before `WHAT COMES NEXT`. Test, `tests/tunnelgoons/test_worldsmith.py:432`
...
- A, the platform: steps 13–21; owns `app/**`, `turn/context.py`, `core/io.py`, `core/views.py`,
  ...
  `tests/loner3e/test_create.py`, `tests/support/{table,loner}.py`.
...
- C, the tests: steps 29–31; owns `tests/**` and `PROPOSALS.md`.
...
14. **D2 A and the page's reach.** New `app/present.py`: `@dataclass(slots=True) class Presentation`
    ...
    `ui/game.py` once (`.title`); `_newest\|def illustrate\|def speak` in `runtime.py` finds nothing.
...
17. **P7, one speaker rule.** `core/views.py`: `spoken`'s refusal (`:247`) becomes `f"nobody here has id
    ...
    (`:252–261`) goes. Test, `tests/core/test_views.py`: `narration_refusal` names a stranger's id and
...
19. **P7, one decode-and-restore.** `core/io.py`: `class Installed(Protocol)` with `scenario: type[
    ...
    `routed(value, engines).scenario`. `runtime.py:69–73,402–405` and `launch.py:70,97–102` pass the
    engines mapping (`engine = engines[state.engine]`; `decode`, `routed` imports in `launch.py` go).
...
22. **P3, one `create_character`.** `Engine.create_character(self, name: str, brief: str, picks: Picks)
    ...
    define `sheet` instead; the four `test_create.py` hold. Grep: `def create_character` finds
    `seam.py` only.
...
24. **P3, one items hook.** `base.py`: `Person.items_title: ClassVar[str] = ""` (a `ClassVar`, so no
    ...
    (id, name, detail; the id is empty for a line that is no item — PROPOSALS' `EntityId` cannot spell
    Breathless's med kit), `items_lines(self) -> str` = `lines_of(f"- {name}" + (f"[{id}]" if id else
    ...
    appends `(sheet.items_title, ", ".join(names))` when `items_title`. Delete `sheet_sections`,
    `panels` and the three `preview_character` overrides
    (`breathless/engine.py:147–172`, `twentyfourxx/engine.py:192–212`, `tunnelgoons/engine.py:135–137`).
...
27. **D5 A, 24XX.** `starting_items` (`twentyfourxx/engine.py:346`) becomes `Pack.starting_items(self,
    ...
    (weapon,)))`); `sheet` calls it. Test: `tests/twentyfourxx/test_world.py:103` moves to
    `tests/twentyfourxx/test_create.py` as `SRD.model_copy(update={"starting_kit": (Kit(name="Comm"),
    Kit(name="Comm"))}).starting_items(Specialty(id="bare", label="Bare", detail="none", skills={}),
    None)` keyed `["comm", "comm-2"]`. Grep: `^def starting_items` finds nothing.
28. **D7 B.** `SceneEngine.pack_options` (`scenes/engine.py:105`) sets `detail=pack.source`; the
    ...
    `tests/core/test_seam.py:128`: `pack_options()[0].detail == "the test"`.
29. **P8, the tests fold into `tests/core`.** `tests/support/table.py`: `SCENE_ENGINE_IDS = (LONER3E,
    ...
    (PROPOSALS calls three identical; the tree shows Loner's inserting a run and the others appending
    an exchange). Grep: `grep -rhoE "def test_\w+" tests/{loner3e,breathless,twentyfourxx,tunnelgoons}
...
31. **The record.** `git rm PROPOSALS.md` (its content is this plan). Grep: `PROPOSALS` in `CLAUDE.md`
    and `.claude/` finds nothing.

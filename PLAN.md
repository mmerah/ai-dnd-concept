...
Eight phases, in order. **Phase 1** puts the four commands on every push and brings `qa/` under the
linters; nothing runs on push today, so every later phase would otherwise be verified by hand.
**Phase 2** closes the tool-argument hole, deletes two dead guards, splits `core/views.py` and the
prompt renderers by owner, evicts the single-family symbols from the shared engine base, and fixes
the five ways the room family diverges from the scene family. **Phase 3** collapses six spellings of
"a thing with a name and a line" into one law. **Phase 4** deletes `Fact.kind`, of which fifty
literals are written and one is read. **Phase 5** moves the per-engine palettes onto the engine,
gives the UI real accessors, computes the view and the history once per turn, and splits
`GameService` into itself, a `Presenter` and a `Roles`. **Phase 6** writes the two missing
playthrough tests and then factors the sheeted person and hiring. **Phase 7** is the consistency
pass: seventeen findings with no other owner. **Phase 8** re-shapes `tests/` to mirror `src/`, a pure
move. Self-standing: an implementer needs this file, `CLAUDE.md` and the code.
...
Saves have no version field. Phase 4 removes `Fact.kind` from every stored fact and Phase 7 moves
the story marks off `Exchange.prompt`, so saves written before those phases no longer parse: the
launcher logs `skipping save ...` and shows the rest (`app/launch.py:99-101`). Nothing migrates.
...
5. **The MCP tool surface stays process-global.** `Runtime.playing()` (`app/runtime.py:410`) cannot
   ...
   every tool call, and `app/mcp.py:63` runs `stateless=True`. Phase 7 rewrites the comment at
   `runtime.py:411` to say that, and to record the upgrade path: if multiplayer or concurrent
   multi-save play ever becomes real, drop `stateless=True`, route by MCP session, and delete
   `playing()`, `Runtime.lock` and `NO_TURN`. Do that only with a JSON-RPC round-trip test in
   `tests/`; today it exists only in `qa/s_mcp.py`.
...
10. **`config.py` stays at the top level, in no layer, and the rule is what is wrong.** `turn/`
    ...
    `("app", "ui")`.
11. **One tool-argument validation site: `Turn.call`.** Both transports validated separately and
    one did it wrong (`app/mcp.py:87` raises `ValidationError` outside its `except Refusal`). The
    refusing code already exists one layer down, in `master_tool`'s inner `call` (`core/tools.py:45`
    calls `parse`), which answers `Input should be a valid dictionary or instance of Attempt` for
    `[1, 2]`, `'hello'` and `5` alike. Phase 2 widens the argument type and deletes both checks.
...
4. **Delete both transport checks.** `app/mcp.py`: drop `_ARGUMENTS` (`:17`) and pass
   `params.arguments or {}` straight to `runtime.call` (`:87`). `app/builtin.py:156-160`:
   `_arguments` becomes `decode(text)` returning `JsonValue`, with the `isinstance` check and its
   `Refusal` gone. One test per transport: a call whose arguments are `[1, 2]` comes back as a tool
   error carrying the refusal, not as an escaping exception.
...
7. **Split prompt rendering by owner.** Each renderer has exactly one caller. Fold `render_master`
   (`turn/context.py:23`) into `turn/run.py` beside `Turn.picture`. Move `render_narrator` (`:47`),
   `render_interjection` (`:60`) and `_picture` (`:81`) into a new `app/roles.py`, beside their
...
8. **Evict the single-family symbols from `engines/base.py`.** All four are used by scene engines
   ...
   `core/facts.py` beside `roll`, which is where a reader looks for it.
9. **One generic `ChangeWorld[C]`.** Add `class ChangeWorld[C](Frozen)` to `engines/base.py` with
   `change: C = Field(discriminator="verb", description=...)`, the description taken verbatim from
   the four copies. Each engine keeps its own `type WorldChange = ...` and writes
   `ChangeWorld[WorldChange]`, so the union stays a written annotation the checker still reads.
   Delete `loner3e/tools.py:63`, `breathless/tools.py:43`, `tunnelgoons/tools.py:22`,
   `twentyfourxx/tools.py:122` and the fifth copy at `tests/core/test_rooms.py:43`. Confirm two
   things before committing: that Pydantic accepts `Field(discriminator=...)` on a type-parameter
   field at parametrisation time — if it does not, rename the four instead
   (`BreathlessChangeWorld`, ...), which removes the collision without losing types — and that
   `tests/core/fixtures/schemas/*/master_tools.json` do not move, since `schema_of` strips `title`.
10. **The family rules suffix becomes a hook.** `scenes/engine.py:96` appends a family `rules.md` to
    ...
    overrides it with a new `engines/rooms/rules.md`, written to say for the room family what
    `scenes/rules.md` says for scenes.
...
13. **`rooms/engine.py:62` says `EXTEND`.** It reads `operations = (MORE_MAP.id,)` while `:176`
...
A CLI sending non-object tool arguments over either transport gets a tool error carrying the
refusal. `turn/context.py` and `rooms/drafts.py` are gone; `core/prompt.py`, `app/roles.py` and
`engines/rooms/rules.md` exist; no module reads a file at import time. `engines/base.py` holds
...
behaviour; `tests/core/fixtures/schemas/` unchanged.
...
1. **Delete `Action`** (`core/views.py:59`). It is field-for-field a `DecisionOption`
   (`core/play.py:62`), which additionally has `min_length=1` on `label` and `detail: str = ""`.
   `PlayerView.action` (`core/views.py:141`) takes `DecisionOption | None`; `MORE_MAP`
   (`rooms/engine.py:48`) and `MOVE_ON` (`scenes/world.py:21`) change type only; `Observed.action`
   (`ui/game.py:56`) follows.
2. **Delete `Named`** (`core/model.py:48`), a class with one use at `:57`. `CharacterHeader` reads
   ...
   `payload` by a `model_validator(mode="before")`. Update `app/launch.py:79-80` and
   `core/io.py:127`, the only readers. The files on disk do not change: the payload still carries
   `name` and `brief`.
3. **Collapse `Rows` and `Sections`** (`core/views.py:21-22`). They are PEP 695 aliases of one
   ...
   str], ...]`. Replace both names everywhere (22 and 30 sites).
4. **Rename `Subject.name`/`brief` -> `label`/`detail`** (`core/views.py:25`). Touches
   `Thing.subject()` (`engines/base.py:92`), the panel builders (`:225-262`), `app/media.py:176,193`
   and the party rendering that becomes `app/roles.py:_picture`. `Thing` keeps `name` and `brief`:
   they are persisted.
...
1. **Make the one reader read a flag.** `_strike` (`loner3e/engine.py:251`) returns
   `tuple[list[Fact], bool]` — the facts and whether the conflict ended. It already knows at `:259`
   (`if hit.luck.current != 0`). The caller at `:205` unpacks the flag and drops
   `any(fact.kind == "conflict_lost" for fact in exchange)`.
2. **Remove `kind`.** Delete the field from `Fact` (`core/facts.py:35`) and the first parameter from
   `Thing.fact()` (`engines/base.py:73`), then every call site: ~60 across the four engines and the
   two families, plus the direct `Fact(kind=...)` constructions (19 in `src/`).
...
Target: about +40 lines net, `app/runtime.py` down from 511 to about 300. The palettes move onto the
engine, the UI stops reaching through the service into the engine, the view and the history are
computed once per turn, and `GameService` gives up two collaborators.
...
1. **The palettes belong to the engine.** `ui/theme.py:26-73` hardcodes `EngineId("loner3e")`,
   ...
   `DiceLook`, and `palette: Palette` to `Engine`'s class attributes (`engines/seam.py:42`, beside
...
2. **Seed the CSS from the registry.** `theme.seed(palettes: Mapping[EngineId, Palette])` stores
   ...
   Delete the hex-value assertion at `tests/ui/test_theme.py:67`, which is a wiring test over a data
   table; replace it with one that every built engine has a block in the generated CSS, so a fifth
...
5. **Forbid the reach.** Extend `tests/core/test_package_boundary.py` to fail on an attribute access
   `.engine` on a `GameService` inside `src/aidm/ui/`. Today it checks imports only, which is why
   twelve method calls slipped through a passing ban.
6. **Build the narrator's view once per turn.** `app/runtime.py:274` builds a `NarratorView` to
   ...
   `Turn.finish` (`turn/run.py:125`) follows, as do `GameService.open` and `GameService.interject`.
...
8. **One `PlayerView` and one history walk per UI tick.** `poll_turn` (`ui/game.py:374-381`) already
   ...
   `:62, 209, 250, 265, 282, 304, 422, 480` and the history at `:66, 204, 324, 389, 444, 497`.
...
cancels an in-flight illustration. `app/runtime.py` is about 300 lines. Full check green.
...
5. **Spike the mixin, then write it.** Confirm in a throwaway that a mixin composes through
   `Engine[P, G]` -> `SceneEngine[C, P, G, K]` -> concrete under basedpyright. If it does not, fall
   back to a template method on the seam with the same hooks and say so in `PROGRESS.md`.
6. **`engines/hiring.py`.** `Hiring` owns `HIRE`, `HIRE_TOOL`, `hire_target`, `Engine.hire`
   (`seam.py:180`), the HIRE branch of `Engine.unwritten` (`:191`), `Engine.check_request` (`:195`),
   `World.require_hireable` (`engines/base.py:144`, a method whose whole body is a refusal with its
   argument unused), `World.sign_on` (`:147`), `Sheeted` and `ItemSheet`. It contributes
   `operations = (*SceneEngine.operations, HIRE)`, today hand-synced at `breathless/engine.py:73`,
   `tunnelgoons:74`, `twentyfourxx:76`. It absorbs the six-step `advance` block written three times
   (`breathless/engine.py:194`, `tunnelgoons/engine.py:153`, `twentyfourxx/engine.py:303`), leaving
   each engine steps 3 and 5 — its prompt text and its sheet shape, the two that are genuinely
   per-ruleset — as hooks. The three engines opt in; loner3e stops carrying five pieces it never
   uses.
7. **Delete what the mixin now owns** from `engines/seam.py` and `engines/base.py`, and the three
   `!= HIRE` guards at `breathless/engine.py:197`, `tunnelgoons:156`, `twentyfourxx:306`.
...
1. **The story marks stop being player input.** `app/runtime.py:34-37` defines `OPENING_MARK`,
   ...
   at `ui/game.py:212` — and `core/prompt.py:_told` feeds them verbatim into the master and narrator
   prompts as `f"> {exchange.prompt}"`, so the master reads `> (the story begins)` as a player
   action. Add `type Mark = Literal["", "opening", "story", "interjection"]` and
   `Exchange.mark: Mark = ""` to `core/play.py`; `Engine.close` takes the mark and leaves `prompt`
   empty for a marked exchange. `_told` renders a marked exchange as its transcript alone, except
   an interjection, which gets a prefix naming it as a party member speaking unprompted, never a
   `>` line. `ui/game.py:212` tests `exchange.mark`. Delete `MARKS`. Saves written before this phase
   no longer parse; the launcher skips them. Regenerate the four turn goldens.
2. **`Refusal` is not for programmer errors.** `engines/registry.py:11` and `engines/seam.py:54`
   raise `Refusal` through `require_unique` for what only a code bug can cause — duplicate engine
   ids, duplicate tool names — both at app start, where `app/runtime.py:414`'s bare `ValueError` is
   the right shape. Raise `ValueError` at both, keeping `require_unique` itself as it is: it is
   genuinely dual-use and is also called at tool boundaries.
3. **Module layout, three sites.** `CLAUDE.md` orders a module: imports, constants, classes, public
   functions, private functions. `ui/game.py:597-637` — six public functions after the private
   `_card:539`, `_dice_group:551`, `_bubble:569`, `_inline_status:582`, `_clock:592`.
   `ui/app.py:60` — `class LaunchForm` after the public `start:47`. `loner3e/tools.py:102` — `TOLD`
   is a plain dict, not a constant built from a class, so it belongs at the top. These are the only
   three in `src/aidm`.
...
7. **`Roles.each` stops enumerating by hand.** `config.py:89-94` lists the three roles that
   `for_name` (`:80-87`) already lists. Write it as
   `tuple((name, self.for_name(name)) for name in get_args(Role))`. The exhaustive `match` in
   `for_name` stays: it type-checks. `Providers.for_name` (`:107`) stays as it is.
...
11. **Name the two notions of in flight.** `GameService.busy` is `phase is not None`
    (`app/runtime.py:91`); `Runtime.playing()` filters on `turn is not None` (`:412`).
    `busy_refusal` uses the first, the tool surface the second, and they diverge during `open()` and
    `_generate`, which set a phase without a turn. Intentional, never stated: one line on each
    saying which question it answers.
12. **Rewrite the comment at `app/runtime.py:411`.** It reads *"A second turn in flight has no
    owner: the tool surface is shared"*, which describes a state the code prevents elsewhere, so a
    reader cannot tell the raise is unreachable. Say instead: this app is single-player;
    `busy_refusal` (`:424`) stops a second turn opening and `Runtime.lock` (`:383`) serialises every
    tool call, so this is a cannot-happen guard — and if multiplayer or concurrent multi-save play
    ever becomes real, the answer is to drop `stateless=True` (`app/mcp.py:63`), route by MCP
    session, and delete `playing()`, `Runtime.lock` and `NO_TURN`.
13. **Name the interjection die.** `app/runtime.py:171` rolls `self.rng.randint(1, 10) <=
    INTERJECTION_ODDS[candidate.chattiness]` inline, the one die that does not go through
    `core.facts.roll`. Move it into a private `_speaks(self, candidate) -> bool` beside
    `INTERJECTION_ODDS` with one line saying why it is not the game's dice: it decides whether to
    spawn a narrator, changes no state and lands no fact.
...
16. **Free functions whose first argument is one of our objects become methods.**
    `hire_target(request: Generation)` (`engines/base.py:215`, moved to `engines/hiring.py` in
    ...
    caller. The three `PlayerView` functions at `ui/game.py:597,602,628` stay free: they are
    unit-tested on their own, which the rule as written does not carve out — add that carve-out to
    the rule in `CLAUDE.md`.
17. **The name collisions the vocabulary law does not reach.** Rename so each name means one thing:
    `Check` (`core/model.py:24`, a `Callable` alias) against `breathless/tools.py:50` (a tool-arg
    model) — `app/spawn.py:17` imports one and `breathless/engine.py:27` the other; `Item`
    (`rooms/world.py:17`, `breathless/world.py:30`, `twentyfourxx/world.py:39`), three unrelated
    shapes two engine files import under one word; `commit` (`core/model.py:117`,
    `engines/seam.py:132`, `app/runtime.py:341`), of which `app/runtime.py:126` reads
    `self.commit(self.engine.commit(draft))` — two meanings in one expression; `unwritten`
    (`engines/base.py:108` returning forbidden fields, `engines/seam.py:189` returning a failure
    card); `require_actor`, which returns a tuple in `tunnelgoons/world.py:83` and one value in
    `breathless:194` and `twentyfourxx:203`; `starting_items` (`rooms/engine.py:85` a hook,
    `tunnelgoons/world.py:63` a method, `twentyfourxx/engine.py:507` a free function); `_Choice`
    (`app/builtin.py:50`, `app/media.py:139`), both parsing an OpenAI-shaped reply in one package;
    and `Driver.parse` (`app/spawn.py:43`), which shadows the project-wide `parse` imported at
    `:15`.
...
`Runtime.playing()` learns why its raise cannot fire and what to do if it must. One name, one
meaning, for the eight collisions listed. Full check green; the golden diff is the marks and
nothing else.

---

## Phase 8 — The test re-shape

Target: 0 lines changed beyond imports. ~30 files move so that `tests/` mirrors `src/`. This phase
is last and it is a pure move: nothing else is in its window, so the diff is verifiable by reading
the file names.
...
2. **Move the six that test `app`** out of `tests/core/`: `test_builtin.py`, `test_game_service.py`,
   `test_mcp_lifespan.py`, `test_media.py`, `test_spawn.py`, `test_speech.py`.
...
4. **Move the six that test `engines`**: `test_rooms.py`, `test_scenes.py`, `test_seam.py`,
   `test_engines_base.py`, `test_dice.py`, `test_integrity_boundaries.py`.
5. **Move `tests/ui/test_launcher.py` to `tests/app/`.** Its 313 lines import `app`, `config`,
   `core` and `engines`, and no `ui` at all.
6. **What stays in `tests/core/`:** `test_config.py`, `test_documents.py`, `test_engine.py`,
   `test_golden_schemas.py`, `test_golden_turn.py`, `test_master_tools.py`,
   `test_package_boundary.py`, `test_store.py`, `test_views.py`, and `fixtures/`. The goldens keep
   their path, so no fixture is rewritten.

...
Phase 1 is every accepted proposal except D4: the fixes, the decided requirements that do not
touch the save format, and the consistency sweep. Phase 2 is D4 alone, because it changes the
scenario and save format and is done together with pack authoring, which this plan does not
design.
...
About three days. Steps 1 to 7 are core and engines, 8 to 13 are app and ui.

### Steps

1. **N8 and the optional section (Q4).** In `core/prompt.py`, `type Pairs` becomes
   `type Sections = tuple[tuple[str, str], ...]`, and `core/views.py` declares its own
   `type Rows = tuple[tuple[str, str], ...]` instead of importing `Pairs`. `core/prompt.py` also
   gains, under `sections`:
   ...
   sections, so `TwentyfourxxWorld.sheet_rows` ends instead with
   `return (*rows, ("Gear", gear)) if gear else rows`.

2. **N7: `Rolled` carries its dice once.** In `core/facts.py`, drop the `rolled` field from
   `Rolled`; `kept` and `total` read `self.event.rolled`; `_rolled` returns
   ...
   `subject_face, action_face = rolled.event.rolled`. In `tests/core/test_dice.py` the four
   `rolled.rolled` reads become `rolled.event.rolled`, and one new test asserts `face` gives the
   single die and raises `ValueError` on a two-die pool. `DiceEvent.rolled` is untouched, so
   `ui/dice.py`, `ui/game.py` and the engine tests that read `dice[0].rolled` do not change.

3. **F3, F4 and F1: one publication, one `except` per boundary, one claim helper.** In
...
4. **D1: strict validation.** `Frozen`, `Mutable` and `Loose` (`core/entities.py`) gain
   `strict=True`, and beside `parse`:
   ...
   `GameService.resume` (`app/runtime.py`, step 8 builds it), `_save_option` (`app/launch.py`),
   `Table.saved` (`tests/support/table.py`), and `engine.restore(decode(...))` in
   `tests/core/test_store.py`, `tests/engines/test_integrity_boundaries.py` (three),
   `tests/engines/test_seam.py`, `tests/loner3e/test_engine.py`, `tests/turn/test_decisions.py`,
   all of which pass the text.

   A master's tool call arrives already decoded, so `master_tool`'s `call`
   (`core/tools.py`) re-encodes it once: `parse_json(args, json.dumps(raw))`. Everything else
   keeps `parse`: `Game.commit`, `routed`, `SceneWorld.opening`, `RoomWorld.opening`,
   `BreathlessEngine.answer`'s `TakeLoot` (its option args carry no tuple).

   `Settings` is a `BaseSettings` and stays lax, but its six nested models inherit `Frozen` and
   read env strings, so `config.py` gains, above `ProviderConfig`:
...
5. **Q3, N4, N10 and the rest of Q4 in the engines.** Helpers that mutate become methods:
   ...
   `Adventurer.rows` keeps its override (Health then the sheet's rows) and `Npc.rows` keeps
   its. The bound is a model, not a protocol, because pydantic cannot build a schema for a field
   whose type parameter is bound by a `Protocol`. One new test in
   `tests/engines/test_engines_base.py`: a `Sheeted` person with no sheet has no rows.

   N10: `Hiring` (`engines/hiring.py`) stops being a `Callable` alias and the `hiring` factory
   goes:

   ```python
   @dataclass(frozen=True, slots=True)
   class Hiring[G: Game[Any], M: Person, A: BaseModel]:
       ...
       async def write(self, draft: G, member: M, terms: str, worldsmith: WorldsmithAnswer) -> str:
           answered = await worldsmith(
               self.prompt(draft, member, terms), self.answer, self.check(draft)
           )
           return self.install(member, answered)
   ```

   with a named `def _no_check[G, A: BaseModel](_draft: G) -> Check[A]` above it in place of the
   lambda returning a lambda. `Engine.hiring(self) -> Hiring[G, M, Any] | None`
   (`engines/seam.py`) keeps the seam's erasure: it never names the answer model, the same reason
   `AnyEngine` is spelled with `Any`. `Engine.write_hire` calls `hiring.write(...)`. The three
   overrides return the exact type: `Hiring[BreathlessGame, Survivor, SheetDraft]`,
   `Hiring[TunnelGoonsGame, Npc, AbilitiesDraft]`,
   `Hiring[TwentyfourxxGame, Crewmate, SheetDraft]`, each constructed with keywords.
   ...
   where `expected` includes the player (`_named(None)` is "the player", a private function under
   the classes) and `Counter` leaves the module if nothing else uses it. In
...
6. **D3 option B is infeasible.** basedpyright refuses a type parameter whose bound is generic
   ("TypeVar bound type cannot be generic", in both the PEP 695 and the `TypeVar` spelling), so
   `SceneEngine[C, W: SceneWorld[C], G, K]` does not check, and every bound that does check
   introduces a new `Any`. The three `world_of` overrides in `loner3e/engine.py`,
   `twentyfourxx/engine.py` and `tunnelgoons/engine.py` stay as they are. No diff; do not attempt
   it.

7. **D5: pin the shallow freeze.** One test per family, asserting the authored scenario payload
   is unchanged after the world begun from it is mutated: in `tests/engines/test_scenes.py` for
   the scene family and `tests/engines/test_rooms.py` for the rooms family. Each begins a game,
   mutates the world on the state (a cast or dweller field), and asserts the `Scenario` object's
   `payload` still reads as it did. No source change; `validate_assignment` stays off.

8. **N1, N2, F2, F5 and N14: how a session is built and guarded.** In `app/runtime.py`:
   ...
   - `GameService.open` drops its `if not self.unopened(): return` guard (`Runtime.open`
     below holds it); `unopened` stays, because `GamePage.build` reads it to decide whether to schedule
     the opening.
   ...
   `restart` calls `await self.runtime.restart(self.session)` then `poll_turn()` then `_open()`;
   `_open` becomes `await self._run(lambda: self.runtime.open(self.session))` with its own guard
   gone. `Observed` becomes `@dataclass(frozen=True, slots=True, kw_only=True)`, and its two
   construction sites (`Observed.of` and `GamePage.__init__`) pass keywords.
   ...
   Tests: `tests/support/table.py`'s `open_table` builds `Runtime.start(settings, lambda _:
   spawner)`, `play_turn` and `take` go through `table.runtime.play(table.service, ...)` and
   `table.runtime.act(table.service, action, words)`, and `drain` awaits `service.close()`.
   `tests/app/test_game_service.py` builds every `Runtime` through `start`, opens through
...
9. **D2 and N3: the dice and the event loop.** `Turn.begin` (`turn/run.py`) stores
   `rng=deepcopy(rng)`, so a failed turn spends none of the game's dice. In `app/runtime.py`,
   `GameService._turn` sets `self.rng.setstate(turn.rng.getstate())` immediately after its
   `self.save(state)`, with the one-line reason that the dice advance with the turn.
   `GameService` gains `chatter: Random = field(default_factory=Random)` and `_speaks` rolls it
   instead of `rng`, so companion chatter cannot move gameplay rolls. `Runtime.new_scenario`
   awaits `asyncio.to_thread(given_text, meta.premise, document, self.settings.source_max_chars)`.
...
10. **N5: one retry constant, one timeout idiom.** `Roles.master` (`app/roles.py`) loops
    ...
    `process.communicate()`, matching `run_builtin` (`app/builtin.py`); `wait_for` is dropped
    from the imports and the refusal text keeps its shape.

11. **Q2: one upload directory per form.** In `ui/create.py`, `ScenarioForm` gains
...
12. **N13: the CSS leaves `theme.py`.** The body of `STATIC_CSS` moves verbatim into a new
...
13. **N11: the comment and naming sweep.** Shorten to one line each: the comment above
    `create_button` in `CharacterForm.build` (`ui/create.py`); the scroll-area comment in
    `GamePage.build`, the scene-art comment in `scene_header` and the portrait comment in
    `sidebar` (`ui/game.py`); the two-line comment above the palette in `install()`
...
About a day, alongside pack authoring. It changes the scenario and save format, so the shipped
content is regenerated and the goldens are rebuilt.
...
1. **The selection value.** `core/model.py` gains, above `Scenario`:
   ...
   go, because the selection's own validator is the uniqueness check now. `None` is what an
   engine that ships no packs stores, which today is the tunnelgoons scenario.

2. **The engine reads one selection.** In `scenes/engine.py`: `SceneEngine.validate` refuses a
   `None` selection and refuses ids the engine does not install, naming them;
   `SceneEngine.selection(self, state: G) -> PackSelection` returns the validated selection and
   replaces `first_pack`, whose callers (`BreathlessEngine.hire_prompt`,
   `TwentyfourxxEngine.hire_prompt` and `hire_check`) take `self.packs[selection.primary]`;
   `pack_content` loses its `picks` argument and reads the selection;
   ...
   selected packs both define. `TwentyfourxxEngine.resolve_skill` searches the selected packs
   only, and its refusal names them. `Engine.author`, `Engine.build_scenario`,
   `SceneEngine.guidance` (and its three engine overrides) and `Runtime.new_scenario` take
   `packs: PackSelection` in place of `Sequence[Slug]`.
...
3. **The character carries its pack.** `Character.pack: Slug | None = None` (`core/model.py`);
   ...
   character whose `pack` is not in the scenario's selection, naming both. One test per family
   engine in its own `tests/<engine>/test_create.py`, plus one in `tests/engines/test_scenes.py`
   for the refusal.
...
5. **Regenerate the shipped content.** `scenarios/buried-keep/world.json` (tunnelgoons),
   `scenarios/drowned-road/world.json` (breathless), `scenarios/silent-relay/world.json`
   (twentyfourxx) and `scenarios/whispering-vault/world.json` (loner3e), and the four
   `characters/kael/<engine>.json`, are written in the new shape. There is no source document to

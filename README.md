...
The world grows through one request. A scene engine's game master may offer the player the way on at a stopping point, record that they left, or bring a complication down on the place they stand, all through `next_scene`; the page's own buttons are the engine's, so a departure the rules already played needs no second adjudication, and a room engine offers more map when the authored map runs out. A request ends the turn: every later tool call in it is answered with a wait line, and the turn narrates only what landed. The worldsmith then writes and the engine installs, the player reads what the engine says to tell, and a failed write is filed with the request cleared; a reload never finds a pending request.

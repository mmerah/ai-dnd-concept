...
def body(s: Session) -> None:
    ...
    text = clean(page.inner_text("body"))
    offered = page.locator(".game-composer button", has_text="More map")
    labels = [clean(t) for t in page.locator(".game-composer button").all_inner_texts()]
    s.note(f"goons composer buttons: {labels}")

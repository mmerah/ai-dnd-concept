...
def body(s: Session) -> None:
    ...
    s.check("(the story begins)" in text, "opening mark missing")
    ...
    submit(
        page,
        'I fight Mara.\n!roll what="Strike at Mara" actor_id=player question="Does he land it?" opponent_id=mara',  # noqa: E501
    )
    wait_idle(page)
    ...
    s.check(
        "(the story goes on)" in clean(page.inner_text(".game-transcript")), "story mark missing"
    )
    ...
    # 16. Sound toggle.
    icon_before = page.locator(".q-header button i").nth(1).inner_text()
    page.locator(".q-header button").nth(1).click()
    page.wait_for_timeout(300)
    icon_after = page.locator(".q-header button i").nth(1).inner_text()
